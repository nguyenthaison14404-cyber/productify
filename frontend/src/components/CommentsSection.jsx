...
function CommentsSection({ productId, comments = [], currentUserId }) {
  ...
  const deleteComment = useDeleteComment(); // <-- không truyền productId nếu hook bạn không nhận
  ...
  setLocalComments((prev) => [hydrated, ...prev]);
}
      }
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="badge badge-neutral badge-sm">{localComments.length}</span>
      </div>
                  ...
                  <button
                    onClick={() =>
                      ...
                      deleteComment.mutate(
                        { productId, commentId: comment.id },
                        {
                          onSuccess: () =>
